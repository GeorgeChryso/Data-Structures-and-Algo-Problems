...
var findLadders = function(S, E, D) {
    ...
    while(queue.length && time<D.length+1 && !finish ){
        curPaths=[]
        let toDelete=[]
        //find the children of the parents
                ...
                for (let j = 0; j < 26; j++) {
                    ...
                    if (DICTI.has(possibleTransmutation)) {
                        ...
                        if(possibleTransmutation===E){
                            finish=true
                            finalResult.push(T.concat(possibleTransmutation))
                            continue
                        }
                        
                        
                        DICT.delete(possibleTransmutation)
                        toDelete.push(possibleTransmutation)
...
var findLadders=function(S, E, D){
    ...
    while(queue.length){
        ...
        for(let i = 97; i < 123; i++){
            ...
            if( w!==curword&& dict.has(w) ){
 
                  
                if(!visited[w]){
                    queue.push([...curpath,w])
                }

                if(w===E){

                    if(curpath.length<time){

                        time=curpath.length+1
                    }
...
var findLadders = function(beginWord, endWord, wordList) {
    ...
    // Create graph
    let nodes = wordList.concat(beginWord).map(word => ({ word, neighbors: new Set, comeFrom: [], visited: false, paths: null }));


    // nodes is now an array of objects 
    //{
    //    word:word,
    //    neighbors: Set
    //    comeFrom:[],
    ...
    let edges = new Map;//so edges contains {a*m,{word:atm,....}}




    //filling the nodes object
    //for every object
    for (let n = 0; n < nodes.length; n++) {
        let node = nodes[n];//nodes[n] is an object
        let word = node.word;

        //for each letter of every word
        for (let i = 0; i < word.length; i++) {
            ...
            if (neighbors === undefined) {
                edges.set(edge, [node]) ;//so edges contains {a*m,{word:atm,....}}
            } 
            else {
    ...
    return (function pathsTo(node) { 
        ...
        let result = [].concat(...node.comeFrom.map(pathsTo));
...
//so this works but has TLE , BFS to find the minimum 
// length and then after that DFS to find all 
// the sequences that FINISH on that length
var findLadders = function(S, E, D) {

    //BrFS, searches the minmum length (level)
    // where E is reached
    var ladderLength = (function(S, E, D) {
    ...
    function DFS(startnode,currLength,dicty,currPath){
        if(currLength==ladderLength)return 

        for (let i = 0; i < startnode.length; i++) {
            for (let j = 0; j < 26; j++) {
                const W= startnode.slice(0,i)
                +String.fromCharCode(97 + j) // all letters of the alphabet (IN ORDER)
                +startnode.slice(i + 1);
                
                if (dicty.has(W)) {
                 let dicto=new Set(dicty)
                 dicto.delete(W)
                    if(W===E){
                        result.push(currPath.concat(E))
                        
                    }

                  DFS(W,currLength+1,dicto, currPath.concat(W) )
                }
