...
//O(NlogN) time with just sorting
var topKFrequent = function(words, k) {
    let freq={}
    for (const word of words)freq[word]=(freq[word]||0)+1
    let entries=[...Object.entries(freq)].sort(([k1,v1],[k2,v2])=>v2-v1==0?k1.localeCompare(k2):v2-v1)
    return entries.slice(0,k).map(([a,b])=>a)
};

//O(NlogK) time using a heap to store a window of the k most frequent elements
var topKFrequent = function(words, k) {
    let freq={}
    ...
    pq.comparator=(a,b)=>b[1]===a[1]?b[0].localeCompare(a[0]):a[1]-b[1]

    for (const [z,v] of Object.entries(freq)) {
        pq.push([z,v])
        if(pq.length()>k)pq.poll()
    }
    let result=[]
    while(pq.length())result.unshift(pq.poll()[0])
