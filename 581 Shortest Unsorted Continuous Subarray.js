...
// HOW TO IMPLEMENT MONOTONIC QUEUE HERE????
var findUnsortedSubarray = function(A) {
  class dequeue {
    constructor() {
      this.dq = [];
    ...
    pushy(i) {
      ...
      while (this.dq.length && A[i] <= A[this.dq[this.dq.length - 1]]) {
        var temp = this.dq.pop();
      }
     
      this.dq.push(i);
      if (temp!==undefined) {
        
        if(this.start == -1)this.start = temp;
        this.end=i
  ...
  console.log(deq1.start,deq1.end)
